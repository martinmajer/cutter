...
Compiler.prototype.compileMacro = function(token) {
    ...
    if (macro.type == Macro.Type.SINGLE) {
        this.addLeadingSpaces();
        this.output += "__m[\"" + addSlashes(name) + "\"].render($.__m, __w, " + mainParam + ", " + params + ");\n";
    }
    else if (macro.type == Macro.Type.PAIRED) {
        this.addLeadingSpaces();
        this.blocks++;
        this.macrosBlockLevels.push(this.blocks);
        this.output += "__m[\"" + addSlashes(name) + "\"].open($.__m, __w, " + mainParam + ", " + params + ");\n";
        this.compileTokens();
        
        this.addLeadingSpaces();
        this.output += "__m[\"" + addSlashes(name) + "\"].close($.__m, __w);\n";
        this.blocks--;
        ...
        if (nextToken.type == TokenType.STATIC) { // static text
            this.output += "__w(__m[\"" + addSlashes(name) + "\"].filter($.__m, " + mainParam + ", " + params + ", \"" + addSlashes(nextToken.content) + "\"));\n";
        }
        else if (nextToken.type == TokenType.WRITE_RAW) {
            this.output += "__w(__m[\"" + addSlashes(name) + "\"].filter($.__m, " + mainParam + ", " + params + ", " + nextToken.content + "));\n";
        }
        else if (nextToken.type == TokenType.WRITE_ESC) {
            this.output += "__w(__e.jsEsc(__m[\"" + addSlashes(name) + "\"].filter($.__m, " + mainParam + ", " + params + ", " + nextToken.content + ")));\n";
        }
        else if (nextToken.type == TokenType.WRITE) {
            if (nextToken.context == TokenHtmlContext.ELEMENT) {
                this.output += "__w(__e.htmlEl(__m[\"" + addSlashes(name) + "\"].filter($.__m, " + mainParam + ", " + params + ", " + nextToken.content + ")));\n";
            }
            else {
                this.output += "__w(__e.html(__m[\"" + addSlashes(name) + "\"].filter($.__m, " + mainParam + ", " + params + ", " + nextToken.content + ")));\n";
            }
        }
        nextToken = this.tokens[this.offset++];
