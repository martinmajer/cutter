...
Compiler.prototype.initWrite = function(name) {
    if (!name) name = "null";
    this.output += "var write = exec_utils.getWrite(" + name + ", snippets, output, parentWrite);\n\n";
}
    ...
    while (this.offset < this.count) {
        ...
        if (token == null) { // EOF
            ...
            if (token.content == "parent") {
                this.output += "parentSectionsArray[parentSectionsIndex](data, sections, snippets, output, write, parentSectionsArray, parentSectionsIndex + 1);\n";
            }
            else {
                this.output += "sections[\"" + token.content + "\"][0](data, sections, snippets, output, write, sections[\"" + token.content + "\"], 1);\n";
            }
        }
        else if (token.type == TokenType.SECTION) {
            this.output += "if (!sections[\"" + token.content + "\"]) sections[\"" + token.content + "\"] = [];\n";
            this.output += "sections[\"" + token.content + "\"].push(";
            this.compileSection();
            this.output += ");\n";
        }
            ...
            this.output += "data[\"" + addSlashes(varName.substr(1)) + "\"] = " + assign + ";\n";
        }
        else {
...
Compiler.prototype.compileSection = function() {
    ...
    this.output += "function(data, sections, snippets, output, parentWrite, parentSectionsArray, parentSectionsIndex) {\n";
    this.initWrite();
    this.addVariables();
    this.handleWhitespaces();
    this.compileTokens();
...
Compiler.prototype.compileCutout = function(name) {
    ...
    this.output += "(function(parentWrite) {\n";
    this.initWrite(name);
    this.addVariables();
    this.handleWhitespaces();
    this.compileTokens();
