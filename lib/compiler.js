...
Compiler.prototype.compileTemplate = function() {
    ...
    this.output += "function run($, __x, __s, __c, __o, __p, __$) {\n";
    this.initWrite();
    this.addVariables();
    this.compileTokens();
    this.output += "}\n\n";
...
Compiler.prototype.compileParent = function(token) {
    ...
    this.output += "__d[\"" + addSlashes(filename) + "\"].run($, __x, __s, __c, __o, __w, " + params + ");\n";
...
Compiler.prototype.compileVirtualCall = function(token) {
    ...
    if (name == "parent") {
        this.output += "__a[__i + 1]($, __x, __s, __c, __o, __w, " + params + ",__a, __i + 1);\n";
    }
    else if (name == "this") {
        this.output += "__a[__i]($, __x, __s, __c, __o, __w, " + params + ",__a, __i);\n";
    }
    else {
        this.output += "__s[\"" + addSlashes(name) + "\"][0]($, __x, __s, __c, __o, __w, " + params + ", __s[\"" + addSlashes(name) + "\"], 0);\n";
    }
}
...
Compiler.prototype.compileSection = function(token) {
    ...
    this.output += "function($, __x, __s, __c, __o, __p, __$, __a, __i) {\n";
    this.initWrite();
    this.addVariables();
    this.compileTokens();
    this.output += "});\n";
...
Compiler.prototype.compileMacro = function(token) {
    ...
    if (macro.type == Macro.Type.SINGLE) {
        this.addLeadingSpaces();
        this.output += "__m[\"" + addSlashes(name) + "\"].render(__x, __w, " + mainParam + ", " + params + ");\n";
    }
    else if (macro.type == Macro.Type.PAIRED) {
        this.addLeadingSpaces();
        this.blocks++;
        this.macrosBlockLevels.push(this.blocks);
        this.output += "__m[\"" + addSlashes(name) + "\"].open(__x, __w, " + mainParam + ", " + params + ");\n";
        this.compileTokens();
        
        this.addLeadingSpaces();
        this.output += "__m[\"" + addSlashes(name) + "\"].close(__x, __w);\n";
        this.blocks--;
        ...
        if (nextToken.type == TokenType.STATIC) { // static text
            this.output += "__w(__m[\"" + addSlashes(name) + "\"].filter(__x, " + mainParam + ", " + params + ", \"" + addSlashes(nextToken.content) + "\"));\n";
        }
        else if (nextToken.type == TokenType.WRITE_RAW) {
            this.output += "__w(__m[\"" + addSlashes(name) + "\"].filter(__x, " + mainParam + ", " + params + ", " + nextToken.content + "));\n";
        }
        else if (nextToken.type == TokenType.WRITE_ESC) {
            this.output += "__w(__e.jsEsc(__m[\"" + addSlashes(name) + "\"].filter(__x, " + mainParam + ", " + params + ", " + nextToken.content + ")));\n";
        }
        else if (nextToken.type == TokenType.WRITE) {
            if (nextToken.context == TokenHtmlContext.ELEMENT) {
                this.output += "__w(__e.htmlEl(__m[\"" + addSlashes(name) + "\"].filter(__x, " + mainParam + ", " + params + ", " + nextToken.content + ")));\n";
            }
            else {
                this.output += "__w(__e.html(__m[\"" + addSlashes(name) + "\"].filter(__x, " + mainParam + ", " + params + ", " + nextToken.content + ")));\n";
            }
        }
        nextToken = this.tokens[this.offset++];
